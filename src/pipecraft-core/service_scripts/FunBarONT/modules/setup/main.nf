...
process unzip_merge_fastq {
    ...
    output:
        tuple val(fastq_path_absolute), val(barcode_name), path(BLASTDB_PATH), path(processing_dir), path("$processing_dir/${barcode_name}.fastq.gz"), emit: data_tuple

    script:
    """
    echo "\$(date '+%Y-%m-%d %H:%M:%S') Processing: ${processing_dir}" | tee -a $processing_dir/processing.log
    # Normalise input to a single gzipped working file. The original raw file is
    # NOT carried downstream, so this is the only fastq.gz in the tuple and can
    # safely keep the plain ${barcode_name}.fastq.gz name.
