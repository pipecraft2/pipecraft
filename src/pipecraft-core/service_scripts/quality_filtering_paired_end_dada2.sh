...
### Run dada2 quality filtering, but check if the input is MIXED orientation (primersCut_out dir)
if [[ -d "/input/primersCut_out/fwd_orient" ]]; then 
    ...
    for folder in /input/primersCut_out/fwd_orient /input/primersCut_out/rev_orient; do
        ...
        Detected MIXED orient sequences as input data; thus note that there were independent runs for fwd_orient and rev_orient inputs.

        Files in 'qualFiltered_out':
        # *_filt.$extension     = quality filtered sequences per sample in FASTQ format.
