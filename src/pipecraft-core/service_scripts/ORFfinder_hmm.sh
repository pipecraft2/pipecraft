...
if [[ $arthropod_hmm == "true" ]]; then
    ...
    checkerror=$(ORFfinder -in $rep_seqs_file \
        ...
        -strand $strand \
        -outfmt 0 \
        -out $output_dir/tempdir_filter_numts/ORFs.aa.fasta 2>&1)
    check_app_error

    #parse
    perl /MetaWorks1.12.0/perl_scripts/parse_orfs4.plx \
                    $output_dir/tempdir_filter_numts/ORFs.nt.fasta \
                    $output_dir/tempdir_filter_numts/ORFs.aa.fasta \
                    $output_dir/tempdir_filter_numts/ORFs.nt.fasta.filtered \
                    $output_dir/tempdir_filter_numts/ORFs.aa.fasta.filtered

    #hmmscan
    checkerror=$(hmmscan \
                --tblout $output_dir/tempdir_filter_numts/hmmscan.txt \
                /MetaWorks1.12.0/bold.hmm \
                $output_dir/tempdir_filter_numts/ORFs.aa.fasta.filtered 2>&1)
    check_app_error

    #parse
    perl /scripts/submodules/filter_hmm.plx \
                $output_dir/tempdir_filter_numts/hmmscan.txt \
                $output_dir/tempdir_filter_numts/ORFs.aa.fasta.filtered \
                > $output_dir/$in_name.ORFs.HMM.list.txt
    
    #get HMM filtered sequences
    checkerror=$(seqkit grep --quiet -w 0 \
                -f $output_dir/$in_name.ORFs.HMM.list.txt $rep_seqs_file \
                > $output_dir/$in_name.ORFs.HMM.fasta 2>&1)
    check_app_error
    #write out HMM-trashed sequences
    checkerror=$(seqkit grep --quiet -v -w 0 \
                -f $output_dir/$in_name.ORFs.HMM.list.txt $rep_seqs_file \
                > $output_dir/$in_name.NUMTs.fasta 2>&1)
    check_app_error
    checkerror=$(seqkit seq --quiet -n \
                $output_dir/$in_name.NUMTs.fasta \
                > $output_dir/$in_name.NUMTs.list.txt 2>&1)
    check_app_error
    echo "# HMM scan DONE | "
fi
...
# $in.NUMTs.list.txt     = list of sequence identifiers of the above file.

Core commands -> 
ORFfinder -in $rep_seqs_file_path -g $genetic_code -s $start_codon -n $ignore_nested -strand $strand -out ORFs.amino_acid_seqs.fasta
hmmscan --tblout hmmscan.txt /MetaWorks1.12.0/bold.hmm ORFs.amino_acid_seqs.fasta \n" >> $output_dir/README_ORFfinder.txt
fi
